...
// ⚡ Bolt: Wrapped mapped inline elements in React.memo to prevent O(N) re-renders across the entire collection.
const SceneItem = React.memo(({ scene, onTrigger }: SceneItemProps) => (
  <button
...
// ⚡ Bolt: Wrapped mapped inline elements in React.memo to prevent O(N) re-renders across the entire collection.
const ClipItem = React.memo(({ clip, onTrigger }: ClipItemProps) => (
  <button
        ...
        {matrix.scenes.map(scene => {
          ...
          return (
            <div key={scene.id} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <SceneItem scene={scene} onTrigger={onSceneTrigger} />
              {sceneClips.map(clip => (
                <ClipItem key={clip.id} clip={clip} onTrigger={onClipTrigger} />
              ))}
            </div>
          );
        })}
