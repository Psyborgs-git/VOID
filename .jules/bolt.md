...
## 2026-05-19 - React Performance Pattern for Lists
**Learning:** Found a major performance bottleneck where mapped elements in matrices like SessionView render inline. This causes O(N) re-renders across the entire collection whenever any element triggers a re-render.
**Action:** Extract mapped inline elements into their own React.memo wrapped components to prevent unnecessary re-renders across the entire collection.
