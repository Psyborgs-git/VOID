...
## 2026-05-12 - WebAudio TypedArray Caching
**Learning:** In high-frequency Web Audio contexts, allocating a new `Float32Array` in `getAnalyserData` continuously results in massive garbage collection spikes, ultimately causing frame rate drops.
**Action:** Always cache and reuse `TypedArray` instances indexed by stable identifiers (e.g., track IDs), and make sure to explicitly remove them during resource teardown (e.g., `deleteTrack`) to prevent memory leaks.
