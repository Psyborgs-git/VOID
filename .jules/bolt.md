...
## 2026-05-22 - Extract React.memo components for SessionView matrix
**Learning:** Found that inline mapped elements inside large collections like the SessionView matrix cause O(N) re-renders for every clip and scene on any parent update.
**Action:** Extracted inner mapped elements (like Scene columns and clip buttons) into their own standalone React.memo components to halt the re-render chain early.
